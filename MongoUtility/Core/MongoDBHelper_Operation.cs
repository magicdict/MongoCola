...
namespace MongoCola.Module
{
	public static partial class MongoDbHelper
	{
			...
			MongoServer mongoSvr)
		{
			...
			String svrKey = strSvrPath.Split("/".ToCharArray())[(int)EnumMgr.PathLv.InstanceLv];
...
//                            }
					...
					case Oprcode.Repair:
                        //其实Repair的入口不在这个方法里面
						CommandHelper.ExecuteMongoDBCommand(CommandHelper.repairDatabase_Command, mongoSvr.GetDatabase(dbName));
						break;
					default:
						break;
				}
			...
			CollectionOptionsBuilder option, MongoDatabase mongoDB)
		{
			...
			String strSvrPath = Common.Utility.GetTagData(strObjTag);
			String svrKey = strSvrPath.Split("/".ToCharArray())[(int)EnumMgr.PathLv.InstanceLv];
			String ConKey = strSvrPath.Split("/".ToCharArray())[(int)EnumMgr.PathLv.ConnectionLv];
			if (mongoDB != null) {
...
//                    }
		...
		public static Dictionary<String, String> GetShardInfo(MongoServer server, String Key)
		{
			...
			if (server.DatabaseExists(ConstMgr.DATABASE_NAME_CONFIG)) {
				MongoDatabase configdb = server.GetDatabase(ConstMgr.DATABASE_NAME_CONFIG);
				if (configdb.CollectionExists("shards")) {
					foreach (BsonDocument item in configdb.GetCollection("shards").FindAll().ToList()) {
						ShardInfo.Add(item.GetElement(ConstMgr.KEY_ID).Value.ToString(), item.GetElement(Key).Value.ToString());
					}
				}
			}
			return ShardInfo;
		...
		{
			MongoCollection mongoCol = CurrentCollection;
		...
		public static String SaveEditorJavascript(String jsName, String jsCode, MongoCollection jsCol)
		{
			...
			if (QueryHelper.IsExistByKey(jsCol, jsName)) {
				...
				if (String.IsNullOrEmpty(result)) {
					...
					try {
						resultCommand = jsCol.Insert(new BsonDocument().Add(ConstMgr.KEY_ID, jsName).Add("value", jsCode));
					} catch (MongoCommandException ex) {
