	...
	public static partial class CommandHelper
	{
		...
		public static CommandResult ExecuteMongoCommand(MongoCommand mMongoCommand)
		{
			...
			try {
				switch (mMongoCommand.RunLevel) {
					case EnumMgr.PathLv.CollectionLv:
						if (String.IsNullOrEmpty(mMongoCommand.CommandString)) {
							mCommandResult = ExecuteMongoColCommand(mMongoCommand.cmdDocument,RuntimeMongoDBContext.GetCurrentCollection());
						} else {
							mCommandResult = ExecuteMongoColCommand(mMongoCommand.CommandString,RuntimeMongoDBContext.GetCurrentCollection());
						}
						break;
					...
					case EnumMgr.PathLv.InstanceLv:
						mCommandResult = ExecuteMongoSvrCommand(mMongoCommand.cmdDocument,RuntimeMongoDBContext.GetCurrentServer());
						break;
					default:
						break;
				}
				...
				Common.Utility.ExceptionDeal(ex, mMongoCommand.CommandString,
