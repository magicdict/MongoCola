...
namespace MongoUtility.Basic
{
	...
	public static class Utility
	{
		...
		public static void SaveResultToJSonFile(BsonDocument result, string FileName)
		{
			...
			writer.Write(result.ToJson(MongoUtility.Basic.Utility.JsonWriterSettings));
			writer.Close();
		}
		/// <summary>
		///     获得系统JS数据集
		...
		public static List<string> GetJsNameList()
		{
			...
			foreach (BsonDocument item in GetCurrentJsCollection(null).FindAllAs<BsonDocument>()) {
		...
		{
			return size.IsInt32 ? Common.Utility.GetSize((int)size) : Common.Utility.GetSize((long)size);
		}
	}
