	...
	public static class GFS
	{
		...
		public static void OpenFile(String strRemoteFileName, MongoDatabase mongoDB)
		{
			...
			try {
				...
				Common.MyMessageBox.ShowEasyMessage("Error", "No Program can open this file");
			} catch (Exception ex) {
				Common.Utility.ExceptionDeal(ex, "Error", "Exception happend when open file");
			}
		}
		...
		public static UploadResult UpLoadFile(String strFileName, UpLoadFileOption Option, MongoDatabase mongoDB)
		{
			...
			try {
				MongoUtility.Basic.Utility.OnActionDone(new ActionDoneEventArgs(RemoteName + " Uploading "));
