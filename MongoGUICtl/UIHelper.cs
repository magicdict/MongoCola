...
namespace MongoGUICtl
{
	public static partial class UIHelper
	{
		...
		/// <returns></returns>
		public static List<TreeNode> GetConnectionNodes(Dictionary<String, MongoServer> _mongoConnSvrLst, Dictionary<String, MongoConnectionConfig> _mongoConConfigLst)
		{
			Debug.WriteLine("GetConnectionNodes" + Thread.CurrentThread.ManagedThreadId);
			List<TreeNode> ConnectionNodes = new List<TreeNode>();
			foreach (String mongoConnKey in _mongoConnSvrLst.Keys) {
				...
				try {
					...
					MongoConnectionConfig config = _mongoConConfigLst[mongoConnKey];
		...
		static void AuthenticationExceptionHandler(MongoAuthenticationException ex,
			...
			string mongoConnKey)
		{
			...
			if (!configuration.guiConfig.IsUseDefaultLanguage) {
				ConnectionNode.Text += "[" + configuration.guiConfig.MStringResource.GetText(StringResource.TextType.Exception_AuthenticationException) + "]";
				Common.Utility.ExceptionDeal(ex, configuration.guiConfig.MStringResource.GetText(StringResource.TextType.Exception_AuthenticationException), configuration.guiConfig.MStringResource.GetText(StringResource.TextType.Exception_AuthenticationException_Note));
			} else {
				ConnectionNode.Text += "[MongoAuthenticationException]";
				Common.Utility.ExceptionDeal(ex, "MongoAuthenticationException:", "Please check UserName and Password");
			}
			ConnectionNode.Tag = ConstMgr.CONNECTION_EXCEPTION_TAG + ":" + mongoConnKey;
			trvMongoDB.Add(ConnectionNode);
		}

		static string MongoCommandExceptionHandle(MongoCommandException ex,
			List<TreeNode> trvMongoDB, 
			TreeNode ConnectionNode,
			string mongoConnKey)
		{
			...
			if (ex.CommandResult.Response["errmsg"] == "unauthorized") {
				if (!configuration.guiConfig.IsUseDefaultLanguage) {
					...
					Common.Utility.ExceptionDeal(ex, "MongoCommandExceptionHandle:", "Please check UserName and Password");
				}
			} else {
				if (!configuration.guiConfig.IsUseDefaultLanguage) {
					ConnectionNode.Text += "[" + configuration.guiConfig.MStringResource.GetText(StringResource.TextType.Exception_NotConnected) + "]";
					Common.Utility.ExceptionDeal(ex, configuration.guiConfig.MStringResource.GetText(StringResource.TextType.Exception_NotConnected), "Unknown Exception");
				} else {
					ConnectionNode.Text += "[Exception]";
					Common.Utility.ExceptionDeal(ex, "Not Connected", "Unknown Exception");
				}
			}
			ConnectionNode.Tag = ConstMgr.CONNECTION_EXCEPTION_TAG + ":" + mongoConnKey;
			trvMongoDB.Add(ConnectionNode);
		...
		static string MongoConnectionExceptionHandle(MongoConnectionException ex,
			List<TreeNode> trvMongoDB, 
			TreeNode ConnectionNode, string mongoConnKey)
		{
			...
			if (!configuration.guiConfig.IsUseDefaultLanguage) {
				ConnectionNode.Text += "[" + configuration.guiConfig.MStringResource.GetText(StringResource.TextType.Exception_NotConnected) + "]";
				Common.Utility.ExceptionDeal(ex, configuration.guiConfig.MStringResource.GetText(StringResource.TextType.Exception_NotConnected), configuration.guiConfig.MStringResource.GetText(StringResource.TextType.Exception_NotConnected_Note));
			} else {
				ConnectionNode.Text += "[Exception]";
				Common.Utility.ExceptionDeal(ex, "Not Connected", "Mongo Server may not Startup or Auth Mode is not correct");
			}
			ConnectionNode.Tag = ConstMgr.CONNECTION_EXCEPTION_TAG + ":" + mongoConnKey;
			trvMongoDB.Add(ConnectionNode);
			return mongoConnKey;
			...
			EachDatabaseUser UserList)
		{
			...
			if (!String.IsNullOrEmpty(config.DataBaseName)) {
				...
				if (isReplsetMasterServer) {
					...
					MongoClientSettings setting = RuntimeMongoDBContext.CreateMongoClientSettingsByConfig(ref config);
				...
				foreach (String strDbName in databaseNameList) {
					...
					try {
						...
						Common.Utility.ExceptionDeal(ex, strDbName + "Exception", strDbName + "Exception");
