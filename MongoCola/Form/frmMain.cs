...
namespace MongoCola
{
	public partial class frmMain
	{
		...
		private void trvsrvlst_NodeMouseClick(object sender, TreeNodeMouseClickEventArgs e)
		{
			...
			if (e.Node.Tag != null) {
				...
				if (string.IsNullOrEmpty(MongoUtility.Core.RuntimeMongoDBContext._CurrentMongoConnectionconfig.UserName)) {
					...
					lblUserInfo.Text = "UserInfo:" + MongoUtility.Core.RuntimeMongoDBContext._CurrentMongoConnectionconfig.UserName;
				}
				...
				MongoUtility.Core.RuntimeMongoDBContext.SelectObjectTag = string.Empty;
				if (MongoUtility.Core.RuntimeMongoDBContext.SelectObjectTag != null) {
				...
				switch (strNodeType) {
					...
					case ConstMgr.JAVASCRIPT_DOC_TAG:
						statusStripMain.Items[0].Text = "Selected JavaScript:" + MongoUtility.Core.RuntimeMongoDBContext.SelectTagData;
						ViewDataToolStripMenuItem.Enabled = true;
		...
		{
			switch (MongoUtility.Core.RuntimeMongoDBContext.SelectTagType) {
				case ConstMgr.USER_LIST_TAG:
					MongoUtility.Basic.Utility.InitDBUser(MongoUtility.Core.RuntimeMongoDBContext.GetCurrentDataBase());
					ViewDataRecord();
					break;
				case ConstMgr.GRID_FILE_SYSTEM_TAG:
					...
					break;
				case ConstMgr.JAVASCRIPT_TAG:
					MongoUtility.Basic.Utility.InitJavascript(MongoUtility.Core.RuntimeMongoDBContext.GetCurrentDataBase());
					break;
				case ConstMgr.JAVASCRIPT_DOC_TAG:
					ViewJavascript();
					break;
		...
		private void ViewJavascript()
		{
			...
			if (_viewTabList.ContainsKey(MongoUtility.Core.RuntimeMongoDBContext.SelectTagData)) {
				...
				var DataTab = new TabPage(JsName) {
					Tag = MongoUtility.Core.RuntimeMongoDBContext.SelectObjectTag,
					ImageIndex = 1
				};
		...
		private void ViewDataRecord()
		{
			...
			if (_viewTabList.ContainsKey(DataKey)) {
				...
				var mDataViewInfo = new MongoUtility.Aggregation.DataViewInfo {
					...
					IsUseFilter = false,
					IsReadOnly = MongoUtility.Core.RuntimeMongoDBContext._CurrentMongoConnectionconfig.IsReadOnly,
				...
				var DataTab = new TabPage(MongoUtility.Core.RuntimeMongoDBContext.GetCurrentCollection().FullName) {
					Tag = MongoUtility.Core.RuntimeMongoDBContext.SelectObjectTag,
					ToolTipText = MongoUtility.Core.RuntimeMongoDBContext.SelectObjectTag
				};
				...
				var DataMenuItem = new ToolStripMenuItem(MongoUtility.Core.RuntimeMongoDBContext.GetCurrentCollection().Name) {
					Tag = DataTab.Tag,
