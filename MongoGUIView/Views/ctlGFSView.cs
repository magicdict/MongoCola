...
namespace MongoGUIView
{
	public partial class ctlGFSView : ctlDataView
	{
		public ctlGFSView(DataViewInfo _DataViewInfo)
		{
			...
			if (!configuration.guiConfig.IsUseDefaultLanguage) {
				DeleteFileToolStripMenuItem.Text =
                    configuration.guiConfig.MStringResource.GetText(StringResource.TextType.Main_Menu_Operation_FileSystem_DelFile);
		...
		private void ctlGFSView_Load(object sender, EventArgs e)
		{
			...
			cmbListViewStyle.SelectedIndexChanged += (x, y) => {
		...
		private Boolean UploadFolder(DirectoryInfo uploadDir, ref int fileCount, GFS.UpLoadFileOption opt)
		{
			foreach (FileInfo file in uploadDir.GetFiles()) {
				GFS.UploadResult rtn = GFS.UpLoadFile(file.FullName, opt,null);
				switch (rtn) {
